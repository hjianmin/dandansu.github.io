...
window.onload = function(){  
	...
	com.get("middlePlay").addEventListener("click", function(e) {
		if (confirm("确认开始中级水平对弈？")){
			play.isPlay=true ;	
			com.get("chessRight").style.display = "none";
			com.get("moveInfo").style.display = "block";
			com.get("moveInfo").innerHTML="";
	...
	com.getData(getContextPath()+"/js/gambit.all.js",
		function(data){
		com.gambit=data.split(" ");
		AI.historyBill = com.gambit;
	})
	com.getData(getContextPath()+"/js/store.js",
		function(data){
...
com.loadImages = function(stype){
	...
	//绘制棋盘
	com.bgImg = new Image();
	com.bgImg.src  = getContextPath()+"/images/"+stype+"/bg.png";
	
	//提示点
	com.dotImg = new Image();
	com.dotImg.src  = getContextPath()+"/images/"+stype+"/dot.png";
	
	//棋子
	for (var i in com.args){
		com[i] = {};
		com[i].img = new Image();
		com[i].img.src = getContextPath()+"/images/"+stype+"/"+ com.args[i].img +".png";
	}
	
	//棋子外框
	com.paneImg = new Image();
	com.paneImg.src  = getContextPath()+"/images/"+stype+"/r_box.png";
	
	document.getElementsByTagName("body")[0].style.background= getContextPath()+"url(/images/"+stype+"/bg.jpg)";
